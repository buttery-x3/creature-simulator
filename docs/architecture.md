...
| **Announce**       | Perceived available resource not suppressed by `resource_announcement` memory. Baseline above wander and max signal, below bare-threshold _visible_ need scores. Deterministic feature-id pick. Clarity/speaking-position are executor.       |
